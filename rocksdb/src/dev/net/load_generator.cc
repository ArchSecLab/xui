...
namespace gem5
{
    LoadGenerator* LoadGenerator::self = nullptr;
    ...
    LoadGenerator::LoadGenerator(const LoadGeneratorParams &p) : SimObject(p), loadgenId(p.loadgen_id), packetSize(p.packet_size), packetRate(p.packet_rate),
                                                                 ...
                                                                 burstWidth(p.burst_width), burstGap(p.burst_gap), burstStartTick(0), poissonGenerator(p.packet_rate),
    ...
    PoissonGenerator::PoissonGenerator(unsigned packet_rate) : generator(0), generator_low(1), distribution((double)packet_rate), distribution_low((double)4.0 * 89616.0), state_edge(curTick())
    {
    }
