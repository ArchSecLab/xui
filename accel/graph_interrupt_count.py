...
with open("boards/bTracklinpack0",'r') as f:
...
for int_t in int_types:
	...
	for ind,i in enumerate(possibles):
		try:
			with open("boards/b"+int_t+"linpack"+str(i),'r') as f:
				l = find_in_file('uintrs',f)
				if(l == "no 0"):
					continue
				y.append(float(get_column(l,3)))
				x.append(i)
		except:
			pass
	x = np.array(x)
