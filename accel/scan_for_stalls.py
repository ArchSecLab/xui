            ...
            if open_files:
                for open_file in open_files:
                    if file_path in open_file.path:
                        ...
                        print(f"Found process {proc.info['name']} with PID {proc.info['pid']} writing to {file_path}")
                        current_time = psutil.time.time()
                        if (current_time - create_time) > 1800:  # 1800 seconds = 30 minutes
...
def scan_for_stalls(directory):
    for i in range(96):
        latency_path = os.path.join(directory, f"dir{i}/accel_latency")
...
def scan_periodically(directory):
    # I want to kill every process that has g2 in the name
