...
struct fwd_stream **i_fsm;
streamid_t i_nb_fs;
streamid_t i_sm_id;
packet_fwd_t i_pkt_fwd;
...
static void
run_pkt_fwd_on_lcore_interrupt(struct fwd_lcore *fc, packet_fwd_t pkt_fwd)
{
	i_fsm = &fwd_streams[fc->stream_idx];
	i_nb_fs = fc->stream_nb;
  assert(i_nb_fs == 1);
  i_pkt_fwd = pkt_fwd;
  int handler = uintr_register_handler(ui_handler, 0);
	...
	// file descriptor.
//	int descriptor = uintr_create_fd(0, 0);
	asm("stui");
//  _stui();
  printf("User Pci Enabled\n");
  while(!fc->stopped);
}
static void
