...
with open("test_parallel_accel.sh", "r") as f:
	src = Template(f.read())
def substition_vals(short_form:str, max_error: float,request_length: int,pci:int,vmr:float,stddev:float,mode:str="Normal"):
	if pci ==1:
		pci_type = "PCI"
	elif pci == 2:
		pci_type = "SIG"
	...
	return {}
start_line = "#!/bin/bash\n"
request_lengths = [2,20]
#max_errors = [1]
#vmrs = [0.001,0.002,0.003,0.004,0.005,0.01,0.02,0.03]#,2,3]
#stddev = []
#for vmr in vmrs:
#	stddev.append(round(sqrt(vmr*75.0),4))
stddev = [0.0,0.3,0.65,0.72,0.95,1.02,1.25,1.32,1.7,2.0]#[0.2,2.0,5.0,7.0, 10.0,15.0]
#stddev = [7.0]
...
# for max_error in max_errors:
	# for request_length in request_lengths:
		# for i in range(len(stddev)):
			...
			# job_list.append(("T",max_error,request_length,0,vmr,stddev[i],"Normal"))
			# job_list.append(("T",max_error,request_length,1,vmr,stddev[i],"Normal"))
			# job_list.append(("T",max_error,request_length,5,vmr,stddev[i],"Normal"))
			# job_list.append(("A",max_error,request_length,3,vmr,stddev[i],"Normal"))
			## job_list.append(("F",max_error,request_length,4,vmr,stddev[i],"Half"))
			# cnt += 5
...
for min_err in min_errors:
    for request_length in request_lengths[min_err]:
        for i in range(len(stddev)):
            vmr = round(stddev[i]*stddev[i]/request_length,8)
            job_list.append(("T",min_err,request_length,1,vmr,stddev[i],"Half"))
            job_list.append(("T",min_err,request_length,0,vmr,stddev[i],"Half"))
            job_list.append(("T",min_err,request_length,5,vmr,stddev[i],"Half"))
            job_list.append(("A",min_err,request_length,3,vmr,stddev[i],"Half"))
...
while len(job_list):
	j = job_list.pop()
	tests[directory] += src.substitute(substition_vals(j[0],j[1],j[2],j[3],j[4],j[5],j[6]))+"\n"
	directory += 1
